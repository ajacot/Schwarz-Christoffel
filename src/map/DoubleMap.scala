...
class DoubleMap(val to : CRDTMap, val domain : Polygon, 
				...
				val Cs0 : Array[Complex[Double]], val As0 : Array[Complex[Double]]) extends Serializable{
  /*
  ...
  def getDisplay() : Display = {
    val (h_l, v_l) = domain.getGridE(23, 200);
...
object MainDoubleMap extends SimpleSwingApplication {
  ...
  */
  ...
  val (poly, inner_angles) = Polygon.labyrinth(3, 3, 1.0, new Random(159)); // 59, 159
